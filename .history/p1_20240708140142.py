...
# Q. Write factorial program using recursion
# def factorial(n):
...
# result = list(filter(lambda x: all(x % y != 0 for y in range(2, x)), range(2, 25)))
# print(result)

# --------------------------------------------------------------------------------------------
# Q write a function to check the number is prime or not using lambda
...
response = requests.get(url, params=params, headers={"Accept": "application/json", "Authorization": "Basic " + base64.b64encode(f"{username}:{password}".encode()).decode()})
